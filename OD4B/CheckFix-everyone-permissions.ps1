...
<#
    ...
    .PARAMETER OneDriveOnly
        Instructs the script to only consider OneDrive site collections.

    .PARAMETER NoONeDrive
        Instructs the script to only consider site collections that aren't OneDrives.
...
#>
Param(
    [Parameter(Mandatory = $true, Position = 0)][string]$TenantUrl,
    [Parameter(Mandatory = $false, Position = 1, ParameterSetName = "OneDrive")][switch]$OneDriveOnly,
    [Parameter(Mandatory = $false, Position = 1, ParameterSetName = "NoOneDrive")][switch]$NoOneDrive,
    [Parameter(Mandatory = $false, Position = 2)][switch]$ChangePermissions,
    [Parameter(Mandatory = $false, Position = 3)][switch]$IterateLists,
    [Parameter(Mandatory = $false, Position = 4)][switch]$Fix);
